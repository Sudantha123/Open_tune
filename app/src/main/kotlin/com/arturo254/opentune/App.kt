...
class App : Application(), SingletonImageLoader.Factory {
    ...
    override fun onCreate() {
        ...
        // Only plant logging trees in debug builds.
        // In release: zero log overhead, GlobalLog never allocates its 500-entry StateFlow buffer.
    ...
    override fun newImageLoader(context: PlatformContext): ImageLoader {
        ...
        // Memory cache: ~15% of app RAM budget, max 80 MB – keeps album art instant on revisit
        val actMgr = getSystemService(Context.ACTIVITY_SERVICE) as? ActivityManager
        val memClassBytes = (actMgr?.memoryClass ?: 64).toLong() * 1024L * 1024L
        val memoryCacheBytes = (memClassBytes * 0.15).toLong().coerceAtMost(80L * 1024L * 1024L)
        val memoryCache = MemoryCache.Builder()
            .maxSizeBytes(memoryCacheBytes)
            .build()

        return ImageLoader.Builder(this)
            .crossfade(150)                          // shorter fade = snappier feel
...
internal fun resolveImageDiskCacheConfig(maxImageCacheSizeMb: Int?): ImageDiskCacheConfig {
    val sizeMb = maxImageCacheSizeMb ?: 256  // Reduced from 512 MB default; most phones benefit from smaller cache
